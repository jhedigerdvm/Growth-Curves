...
getwd()
#Load dataset
data <- read.csv('Joe/antlers_birthsite/bucks_nofawns.csv', header=T)
head(data)
...
# summary(dmp$bcscm)
# 
# bcs.mean.site.age<-data %>% na.omit() %>% group_by(birthsite, age) %>% summarise(mean=mean(bcscm))
# weight.mean.site.age<-data %>% na.omit() %>% group_by(birthsite, age) %>% summarise(mean=mean(weightkg))
...
max.age.all<-data %>% group_by(animal_id) %>%  summarise(oldest=max(age)) #max age of each individual, only shows row numbers
niners<- which(max.age.all$oldest>=9) #creates a vector for row numbers of individuals living at least 9 years 
...
for (i in 1:n){
   antler[i] ~ dnorm(mu[i], tau) #each antler is a draw from this distribution
   mu[i] <- int + bs.beta[bs[i]] + ageclass.beta[ageclass[i]] 
  }
...
#inits function
inits<- function(){list(ageclass.beta = c(NA,runif(11,1,500)),
                        bs.beta = c(NA,runif(2,1,500)),
                        int = runif(1,0,100),
                        sigma = rlnorm(1))} #log normal pulls just positive values

#parameters to estimate
parameters <- c('int', 'ageclass.beta', 'bs.beta', 'derived_antlers', 'site.diff')
...
lme.jags<- jagsUI(jags.data, inits, parameters, 'lme.jags', n.thin = nt, n.chains = nc, 
                  n.burnin = nb, n.iter = ni)
print(lme.jags)
# write.csv(print(lme.jags$summary), 'lmeoutput.csv')
MCMCtrace(lme.jags)
MCMCplot(lme.jags, horiz = FALSE, params = 'ageclass.beta' ) #quick plot to look at results

#get output into a tibble before you can manipulate in ggplot
posterior.lme<- tidy_draws(lme.jags)
posterior.lme <- posterior.lme[,-c(1:18)]
posterior.lme <- posterior.lme[,-c(37)]

#pivot longer puts them in a tibble format
lme.long <- posterior.lme %>% pivot_longer(everything())

#create age class column and assign 1-12 to appropriate ageclass
lme.long$ageclass <- as.factor(rep(c('1','1','1','2','2','2','3','3','3','4','4','4',
                                     '5', '5', '5', '6','6', '6', '7', '7', '7', '8', '8','8',
                                     '9', '9', '9', '10', '10', '10', '11', '11', '11', 
                                     '12', '12', '12'), nrow(lme.long)/36))

lme.long$bs <- as.factor(rep(c('dmp','ey','wy'), nrow(lme.long)/3))
...
plot_base_lme <- 
  ggplot(data = lme.long, aes(x=ageclass, y=antlers, group = bs))+
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.border = element_blank(),
        axis.line = element_line(),
...
lme.plot<- plot_base_lme +
  stat_pointinterval(aes(color = bs), alpha = 0.7, .width = c(0.5, 0.95), 
                     position = 'dodge') +
...
ggsave('antlers_site.png', lme.plot, bg='transparent', width = 10, height = 10)
